...
contract and M3D-A2 slices 1-7 are done; remaining call-site families are next.
...
| Shadow consolidation | **M3C VERIFIED / M3D-A2 SLICES 1-7 DONE** | root activation `97a300e`; first `ca324fe`; swarm `0c9723d`; operator `2cbc85f`; task-results `4dca9fe`; learning `94e7426`; spend `af3f48a`; home dirs `c3923c9` | remaining A2 families; no cutover |
...
are complete. M3D-A2 slices 1-7 route exec-graph, evidence, goal budgets, swarm
runs, intake drafts, operator state/runs, task results, learning state, and the
global spend receipt writer, instance lease, provider health, and breadcrumbs
only after required manifest-bound activation.
...
3. **Done:** close receipt-forgery bypass by keeping rollback/receipt
   ...
   removed shadow root;
4. **Done:** Tasks 1-4 bounded M3B seams and proof paths, proved one strict
   ...
   **Done:** write M3D rollback/cutover packet from local code and physical-path
   ...
   staged root to become hidden activation. **Done:** slice 2 migrates swarm
   runs and intake drafts while preserving explicit test/legacy roots only
   before activation. **Done:** slice 3 migrates operator state/runs, closes
   trace/ledger file escapes and default-store junction escape, and proves
   activated manual result readback. **Done:** slice 4 migrates task-results and
   refuses invalid activation before subprocess spawn. **Done:** slice 5
   migrates learning state/exchange/projection paths and binds learning's
   evidence, exec-graph, and spend side effects before any activated claim.
   **Done:** slice 6 migrates the global spend writer, fails closed on
   state-root policy/configuration denial before counter or file mutation, and
   preserves non-blocking ordinary local-I/O and Supabase failures.
   **Done:** slice 7 migrates instance lease, provider health, and breadcrumbs;
...
10. **In progress:** M3D-A1 is done at `97a300e`; M3D-A2 slices 1-7 are done at
