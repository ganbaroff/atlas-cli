...
4. **M3D — PACKET WRITTEN / IMPLEMENTATION NEXT — cutover readiness.** Design
   ...
   precedence; 146/146 affected tests and typecheck pass. Slice 2 is complete
   at `0c9723d`: swarm runs and intake drafts preserve their explicit test/legacy
   roots before activation, but required activation ignores those escape hatches
   and routes both stores under the manifest-bound root; exact-tip 39/39 tests
   and typecheck pass. Slice 3 is complete at `2cbc85f`: operator state, traces,
   ...
   Slice 6 is complete at `af3f48a`: the global spend receipt writer preserves
   its legacy env/home location before activation and routes to
   `spend-receipts` after activation. State-root denials occur before counter or
