...
**Current point:** M3 Shadow Consolidation, package M3C written-spec review.
...
3. **M3C — NOW — preserved-state rehearsal design review.** Direction A is
   approved: retain one outside-repository snapshot, rehearse only against that
   copy, and bind cleanup plus rollback evidence before receipt. Written design:
   `docs/superpowers/specs/2026-07-30-m3c-preserved-state-rehearsal-design.md`.
   Implementation waits for adversarial written-spec review and plan.
