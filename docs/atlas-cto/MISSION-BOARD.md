...
| DEBT-10 | `upstreamClass` is hardcoded `'local_mock'` at `model-broker/server.ts:134`, `authorize.ts:133`, `authorize.ts:203`, and `types.ts:70` declares a **single-member union** `readonly upstreamClass: 'local_mock'` — no other value is even expressible. Once the broker's upstream is the C5A-2 egress leg, every broker receipt mislabels a live call as a mock | ATLAS | OPEN — external CTO ruled 2026-08-11 this is **not deferrable debt**: it makes evidence materially false and blocks C5A-2 ACCEPT. Folded into the C5A-2-R1 scope |
| M-2026-08-11-05 | C5A-2-R1 remediation — narrow, pre-scoped, **not started** | QUEUED | ATLAS (build) / Codex (closure) | exactly five items and no more: stream refusal before the network → deterministic outbound deadline → atomic spend reservation → completion-shape validation → truthful `upstreamClass`. No new architecture, no scope growth | none | waits on the formal Codex REJECT of `e552789`. **CEO challenge 2026-08-11, unresolved: should R1 be built at all?** Four of the five items are commodity gateway features (LiteLLM virtual keys / Bifrost) and `ATLAS-OSS-RECONCILIATION-2026-08-08.md` §10 already ruled custom model routing should be replaced. See M-2026-08-11-06. After R1 (or its cancellation) the critical path returns to **C5B0 auth-carrier proof** |
