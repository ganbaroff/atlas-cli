...
| M-2026-08-11-04 | C5A-2 — loopback egress leg (the component that makes a real first chat possible) | NEXT | ATLAS (no elevation) | a local process holds the provider auth, is the broker's single approved `127.0.0.1` upstream, applies the credits ladder (NVIDIA → Vertex → Azure → free → paid last) and a hard spend cap; Hermes `model.base_url` repointed off `https://openrouter.ai/api/v1` onto the broker; one real chat completes with the request chain visible in the evidence ledger | none | **structural finding 2026-08-11:** `upstream-policy.ts` refuses any non-loopback upstream before the broker listens, so C5A can only reach `mock-upstream.ts`. Without C5A-2 the "first chat through the broker" is a chat with a mock |
