...
| P1-DEBT-05 | Provider-invocation idempotency: `runSpendPreflight()` re-invoked the injected provider port on every REPLAYED requestId (ledger-idempotent commit/reserve calls masked it — the ledger never double-charged, but a real provider adapter would receive a second real call) — flagged in the P1-B wave-2 handoff, formally closed this repair | ATLAS | CLOSED — `codex/p1b-spend-cap` commit `0d2a618` (durable `RESERVED -> INVOCATION_STARTED -> COMMITTED \| RELEASED \| REJECTED \| PENDING_RECONCILIATION` claim lifecycle; `ledger.ts`'s `claimProviderInvocation()` performs the atomic compare-and-set inside the existing day-file lock, fsync+rename persisted, strictly before `preflight.ts` ever calls the provider port; 16 new tests in `spend-provider-idempotency.test.ts` incl. a live proof with real concurrent child processes) |
