...
| M-2026-08-11-07 | C5B0 auth-carrier preflight — can Hermes carry a per-request Atlas authorization at all? | DONE | Fable (read-only source inspection) | answered without building anything: **yes, via `llm_request` plugin middleware**, which replaces the outgoing payload per call; Hermes already injects a per-call header itself for a billing marker. Static config headers also work but carry only a fixed token. The observability hook is not the seam — its return value is discarded | `C:\Projects\ATLAS\data\HERMES-C5B0-AUTH-CARRIER-PREFLIGHT-2026-08-11.md` (file:line detail kept off this public board) | state entered 2026-08-11. Consequence: C5B0 is no longer a capability risk, so it stops being a blocker and becomes a cost comparison — feeds M-2026-08-11-06 |
