...
| DEBT-10 | `upstreamClass` is hardcoded `'local_mock'` at `model-broker/server.ts:134`, `authorize.ts:133`, `authorize.ts:203`. Once the broker's upstream is the C5A-2 egress leg, every broker receipt mislabels a live call as a mock | ATLAS | OPEN — deliberately not fixed in the C5A-2 commit: widening it edits audited `authorize.ts` and should happen under review |
